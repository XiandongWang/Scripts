...
IMG_PATH = '../../data/datasets/VisDrone/VisDrone2019-DET-test-challenge/images/'
TXT_PATH = '../../Visdrone-challenge/runs0.01/'

OUT_PATH = '../../Visdrone-challenge/wbf_labels/'
...
for image_id in tqdm(image_ids, total=len(image_ids)):
    ...
    for name in MODEL_NAME:
        ...
        label_list = []
        import cv2
        txt_file = TXT_PATH + name + '/' + image_id.replace('jpg', 'txt')
        jpg_file = IMG_PATH + image_id
    ...
    for i,row in enumerate(boxes):
        ...
        bbox[2] = w
        bbox[3] = h
        out_file.write(str(bbox[0])+','+str(bbox[1])+','+str(bbox[2])+','+str(bbox[3])+','+str(round(scores[i],6))+','+str(int(labels[i]+1))+'\n')
